...
use ale_texture::{Texture, TexturePixel};
...
impl Font {
  ...
  pub fn layout(&mut self, font_size: i32, text: &str, bounds: Option<Vector2<i32>>) -> FontLayout {
    let scale = Scale {
      x: font_size as f32,
      y: font_size as f32,
    };

    let v_metrics = self.intern_font.v_metrics(scale);
    let offset = point(0.0, v_metrics.ascent);

    let glyphs: Vec<_> = self.intern_font.layout(text, scale, offset).collect();
    ...
    for g in glyphs {
      if let Some(bb) = g.pixel_bounding_box() {
        ...
        self.textures.entry(font_raster_key.clone()).or_insert_with(|| {
          let mut raster: Vec<f32> = vec![0.0; width * height];
          g.draw(|x, y, v| {
            raster[x as usize + y as usize * width] = v;
          });

          let texture = Texture::new(TexturePixel::RgbF32(raster), width as u32, height as u32, 1);
