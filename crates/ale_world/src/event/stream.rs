...
impl<T: Sync> EventStream<T> {
  ...
  pub fn stream(&self, reader_entity_id: Id<Entity>) -> EventStreamReader<T> {
  ...
  pub fn send(&mut self, data: T, target: HashSet<Id<Entity>>) {
...
pub struct Holder<T: Sync> {
  data: Option<T>,
  gen: usize,
  target_entity: HashSet<Id<Entity>>,
}
...
impl<T: Sync> EventStreamBuffer<T> {
  ...
  pub fn push(&mut self, data: T, target_entity: HashSet<Id<Entity>>) {
  ...
  fn stream(&self, reader_entity_id: Id<Entity>) -> EventStreamReader<T> {
