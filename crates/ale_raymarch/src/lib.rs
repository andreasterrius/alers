...
use ale_math::{Vector3, RED, GREEN};
use ale_math::{clamp_vec3, InnerSpace};
use ale_mesh::sdf::{MeshSDF, ale_mesh_sdf_distance};
...
pub fn ale_raymarch_sdf_single(camera: &Camera, sdfs: Vec<&MeshSDF> ) -> Vec<(Vector3<f32>, Vector3<f32>)> {
  ...
  for i in 0..x {
    for j in 0..y {
      ...
      for iter in 0..5 {
        ...
        for sdf in &sdfs {
          let dist = ale_mesh_sdf_distance(sdf, curr_pos);
