...
{
    aabb_axis_overlap(
        box_collider1.worldpos().x,
        box_collider1.size().x,
        box_collider2.worldpos().x,
        box_collider2.size().x,
    ) &&
        aabb_axis_overlap(
            box_collider1.worldpos().y,
            box_collider1.size().y,
            box_collider2.worldpos().y,
            box_collider2.size().y,
        )
}

pub fn aabb_collision_box_circle<Box, Circle>(box_collider: &Box, circle_collider: &Circle) -> Option<(Vector2<f32>, f32)>
where
    Box: BoxCollider2D,
    Circle: CircleCollider2D,
{
    ...
    let box_closest_point_to_circle = Vector2::new(
        clamp(
            diff_vec.x,
            -box_collider.size().x / 2.0,
            box_collider.size().x / 2.0,
        ),
        clamp(
            diff_vec.y,
            -box_collider.size().y / 2.0,
            box_collider.size().y / 2.0,
        ),
    );
    ...
    Box: BoxCollider2D,
{
    ...
    for &(ref direction, ref label) in directions.iter() {
        let norm_point = Vector2::new(
            point_in_box.x / (box_collider.size().x + 5.0) / 2.0,
